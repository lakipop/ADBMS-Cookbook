# 📚 ADBMS Cookbook

> **BICT 3rd Year - Final Semester ADBMS Exam Preparation**  
> MySQL-based Advanced Database Management Systems study materials

---

## 📂 Contents

### 🔬 Practical Files

| # | File | Topics |
|---|------|--------|
| 01 | [Basic SQL & Tables](./01-basic-sql-tables-practical.md) | Database creation, CRUD, Joins, Views, Procedures |
| 02 | [Stored Procedures - stuMarks](./02-stored-procedures-practical-stumarks.md) | Procedures, Parameters, Age, Grades |
| 03 | [Stored Procedures - Employee](./03-stored-procedures-practical-employee.md) | Functions, DOB, Location |
| 04 | [Complex Schema & Procedures](./04-complex-schema-procedures-practical.md) | Multi-table schema, Error handling |
| 05 | [UDF Practical](./05-udf-practical.md) | Scalar, Table-valued functions |

### 📖 Theory Files

| # | File | Topics |
|---|------|--------|
| 06 | [UDF Theory](./06-udf-theory.md) | Built-in functions, UDF types |
| 07 | [Views Theory](./07-views-theory.md) | Views, Materialized views |
| 08 | [Stored Procedures Theory](./08-stored-procedures-theory.md) | SP syntax, Parameters, Control flow |

---

## 🎯 How to Use

1. **Review Theory** - Read theory files first
2. **Practice Questions** - Attempt questions without looking at answers
3. **Check Off** - Use checklists to track progress
4. **Practice with Claude** - Ask for answers/explanations

---

## 📊 Progress

- [ ] Basic SQL & Tables
- [ ] Stored Procedures (stuMarks)
- [ ] Stored Procedures (Employee)
- [ ] Complex Schema
- [ ] UDF Practical
- [ ] UDF Theory
- [ ] Views Theory
- [ ] Stored Procedures Theory

---

> **Good luck with your exam!** 🍀
